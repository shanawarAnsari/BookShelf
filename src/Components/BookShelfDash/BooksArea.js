...
const BooksArea = (props) => {
  const classes = useStyles();

  const handleDeleteBook = (code) => {
    
    axios
      .post("http://localhost:5005/deletebook/", {
        code: code,
      })
      .then((resp) => {
          ...
          <Grid item key={card._id} xs={12} sm={6} md={4}>
            <Card className={classes.card}>
              <CardContent>
                ...
                <hr />
                <Typography>Written By: {card.author}</Typography>
              </CardContent>
              <CardActions>
                <Button
                  size="small"
                  color="primary"
                  variant="outlined"
                  onClick={() => handleEditClick(card.code)}
                >
                  Edit
                </Button>
                <Button
                  ...
                  color="primary"
                  variant="outlined"
                  onClick={() => handleDeleteBook(card.code)}
                >
                  Delete
                </Button>
              </CardActions>
